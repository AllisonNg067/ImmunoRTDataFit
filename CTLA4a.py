...
PD_fractions = 1
CTLA4_fractions = 1
file_name = 'RT ' + str(RT_fractions) + ' PD ' + str(PD_fractions) + ' CTLA4 ' + str(CTLA4_fractions) + ' fraction a constant seed.csv'
...
def trial_treatment(i):
  ...
  D = DList[i]
  args = [(i, k, k) for k in range(sample_size)]
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    data = list(executor.map(trial_treatment, range(min(iterations, 1200))))
